...
import nodes.utils.paths as PTH
...
def main(toml_path):
    ...
    output_dir = PTH.REPO_PATH / 'nodes' / 'utils' / 'messages'
    ...
    for message_name, fields in messages.items():
        ...
        with open(class_file, 'w') as f:
            f.write(f"from nodes.utils.messages.mqtt_message_base import MqttMessageBase\n")
    ...
    for message_name, fields in messages.items():
        class_name = message_name.upper() + '_MSG'
        module_name = f"nodes.utils.messages.{class_name.lower()}"
    ...
    with open(PTH.REPO_PATH / 'nodes' / 'utils' / 'topic_to_message_type.py', 'w') as f:
        ...
        for topic, message_class in topic_to_message_map.items():
            if message_class not in imported_classes:
                f.write(f"from nodes.utils.messages.{message_class.__name__.lower()} import {message_class.__name__}\n")
