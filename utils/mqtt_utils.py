...
from .messages.mqtt_message_base import MqttMessageBase
...
class MQTTPublisher():
    ...
    def publish_msg(self, topic, msg: MqttMessageBase = None):
        # check if the message class is in the topic_to_message_map and matches the topic
        try:
...
class MQTTSubscriber(Thread):
    ...
    def clear_message_queue(self):
        """Clear all unread messages from the queue"""
        num_messages = len(self.latest_messages)
        if num_messages > 0:
            logging.debug(f"Clearing {num_messages} messages from queue")
        for topic in self.latest_messages:
            self.latest_messages[topic]['read'] = True
        self.latest_messages = {}
